KISSY.add(function(S){
  ...
  var module = this;
  var root = module.fullpath.split('/').slice(0, -1).join('/');
  var app = angular.module(module.name, [], angular.noop);


  var RichTable = function(node, config){
          ...
          scope.fetch = function(page){
            ...
            for(var i = 0, l = scope.column_list.length; i < l; i++){
              ...
              for(var j = 0, ll = scope.obj.length; j < ll; j++){
                if(scope.obj[j][c + '_rowspan'] == 0){continue}
